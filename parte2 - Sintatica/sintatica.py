...
def p_programa(p):
    ...
    global programa
    global id
    programa = Node(str(id) + " programa", children = [p[1]])
    id +=1
    p[0] = programa
...
def p_parametro(p):
    ...
    global id
    p[0] = Node(str(id) + " tipo" + str(p[2]) + str(p[3]), children = [1] )
    id+=1
